...
async function main() {
  ...
  parentServer.setRequestHandler(ListToolsRequestSchema, async () => {
    try {
      console.error('list tools spawn')
      ensureChildRunning()
  ...
  parentServer.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    console.error('before ensureChildRunning')
    ensureChildRunning()
    console.error('calling child tool:', name, args)
